...
<div class="row">
    <div class="col-sm-12">
        <div class="main-title-00">
            ...
            @endif
            <h4 class="page-title">المقاسات</h4>
        </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class=" main-btn-00">
                        <!-- Responsive modal -->
                        <a href="{{ route('admin.size.create') }}" class="btn btn-default waves-effect">اضافه مقاس +</a>
                    </div>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table data-toggle="table" data-search="true" data-show-columns="true" data-sort-name="id" data-page-list="[8, 16, 32]" data-page-size="8" data-pagination="true" data-show-pagination-switch="true" class="table-bordered ">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$size->name}}</td>
                            <td>{{$size->status === 1 ? 'مفعل' : 'غير مفعل'}}</td>

                            <td class="actions">
                                <a href="{{ route('admin.changeStatus',[$size->status,'sizes',$size->id]) }}" class="btn btn-{{$size->status == 1 ? 'secondary' : 'dark'}} waves-effect" title="الحالة"> {{$size->status == 1 ? 'إبطال' : 'تفعيل'}}</a>
                                <a href="{{ route('admin.size.edit',$size->id) }}" class="btn btn-success waves-effect" title="تعديل">تعديل</a>
                                <button type="button" class="btn btn-danger waves-effect" data-toggle="modal" data-target="#{{$size->id}}delete" title="حذف">حذف </button>
                            </td>
                        </tr>

                        <div id="{{$size->id}}delete" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
